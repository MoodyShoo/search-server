...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words) : stop_words_(MakeUniqueNonEmptyStrings(stop_words)) {
        for (const auto& word : stop_words_) {
            if (!IsValidWord(word)) {
                throw std::invalid_argument("В стоп словах есть запрещенные символы");
            }
        }
    }
...
private:
    ...
    std::vector<int> document_id_by_add_order;
    ...
    static bool CheckForTwoMinuses(const std::string& word);
