        ...
        for (const auto& word_in_query : query_words.plus_words) {
            if (word_to_documents_freq_.count(word_in_query)) {
                ...
                for (const auto& word_in_document : word_to_documents_freq_.at(word_in_query)) {
                    ...
                    document_to_relevance[id] += word_in_document.second * idf;
                }
            }
        }

        for (const auto& word : query_words.minus_words) {
            if (word_to_documents_freq_.count(word)) {
                for (const auto& docum : word_to_documents_freq_.at(word)) {
                    document_to_relevance.erase(docum.first);
                }
            }
        }
